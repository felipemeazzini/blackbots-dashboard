...
export default function AdThumbnail({ src, alt = "", size = "sm" }: AdThumbnailProps) {
  ...
  return (
    ...
    >
      ...
      />
      {showPopup && (
        <div className="absolute z-50 left-0 top-full mt-2 p-1 bg-bg-surface border border-border rounded-xl shadow-2xl">
          <img
            src={src}
            alt={alt}
            className="w-64 h-64 rounded-lg object-contain bg-bg-primary"
          />
        </div>
      )}
    </div>
